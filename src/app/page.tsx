...
// Quotes rotation
const QUOTES = [
  { text: "Your placement officer can't do what our AI does in 60 seconds.", emoji: "💀" },
  { text: "Gharwale poochenge 'Beta job lagi?' — You say 'RADAR laga rakha hai.'", emoji: "😎" },
  { text: "While you sleep, RADAR scrapes. While they scroll LinkedIn, you apply first.", emoji: "⚡" },
  { text: "50 companies. 1 click. 0 recruiter ghosting. Math checks out.", emoji: "📊" },
  { text: "Your resume isn't bad. Your job search strategy was. Until now.", emoji: "🎯" },
  { text: "We don't find jobs. We intercept them before they go viral.", emoji: "🔥" },
];
    ...
    <main className="relative min-h-screen bg-grid scanlines">
      ...
      <nav className={'fixed top-0 left-0 right-0 z-50 transition-all duration-500 ' + (scrolled ? 'py-2.5' : 'py-4')}
        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #8B5CF6, #00FFD4)', boxShadow: '0 0 15px rgba(0,255,212,0.3)' }}>
      ...
      <section id="how" className="relative py-28 z-10 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { n: '01', icon: '🎯', title: 'Tell Us Your Dream', desc: 'Upload resume. Pick dream companies from 50+ firms. Set location. That\'s it.', c: '#8B5CF6' },
              { n: '02', icon: '🔬', title: 'AI Scans Everything', desc: 'Our engine hits ATS APIs every hour. AI embeddings match jobs to YOUR profile — not keywords, understanding.', c: '#00FFD4' },
              { n: '03', icon: '⚡', title: 'Apply Before Anyone', desc: 'Get ranked matches across Dream Companies, AI Picks, and Startup Gems. Posted date. Expiry alerts. One-click apply.', c: '#00FF88' },
            ].map((item, i) => (
      ...
      <section className="relative py-28 z-10 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: '🏢', title: 'Direct ATS Scraping', desc: 'Greenhouse, Lever, Ashby APIs. Real listings, zero duplicates, structured data.', a: '#00FFD4' },
              { icon: '🧠', title: '384D AI Matching', desc: 'Semantic embeddings compare your skills vs every JD. Not keywords — real understanding.', a: '#8B5CF6' },
              { icon: '📍', title: 'Location Intelligence', desc: 'Haversine distance sorting. Freshers in Chandigarh see Gurugram jobs first.', a: '#00FF88' },
              { icon: '📅', title: 'Date & Expiry Alerts', desc: 'See when each job was posted and when it expires. Red alerts for closing soon.', a: '#FF3D71' },
              { icon: '🏠', title: 'Remote-First Filter', desc: 'One click: Remote, Hybrid, In-office. Color-coded badges. Instantly clear.', a: '#FFB700' },
              { icon: '⚡', title: 'Hourly Auto-Scan', desc: 'GitHub Actions cron every hour. New jobs appear before they hit LinkedIn.', a: '#00FFD4' },
            ].map((f, i) => (
          ...
          <FadeIn>
            <div className="glow-card p-1 overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {[
                    ['AI Resume Match', '✅ 384D', '❌', '❌'],
                    ['Direct ATS Data', '✅', '❌', '❌'],
                    ['Hourly Scan', '✅', '❌', '❌'],
                    ['Fresher-First', '✅', '❌', '⚠️'],
                    ['Location Sort', '✅', '⚠️', '⚠️'],
                    ['Free to Start', '✅ 3 Searches', '✅', '✅'],
                    ['Mini-Games 🎮', '✅', '❌', '❌'],
                  ].map((row, i) => (
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto">
            <FadeIn><div className="glow-card p-8">
              <div className="relative z-10">
                ...
                <ul className="space-y-3 mb-8">
                  {['All 3 job sections', '5 companies per section', '3 total searches', 'AI match scoring', '2 mini-games while loading'].map((f, i) => (
                    <li key={i} className="flex items-center gap-3 text-sm" style={{ color: '#94A3B8' }}>
                      <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px]" style={{ background: 'rgba(0,255,136,0.1)', color: '#00FF88' }}>✓</span>{f}
                    </li>
                  ))}
                </ul>
                <Link href="/sign-up" className="block w-full py-3.5 text-center text-sm font-semibold rounded-xl transition-all hover:bg-white/5"
                  style={{ border: '1px solid rgba(255,255,255,0.08)', color: '#94A3B8' }}>Try Free →</Link>
              </div>
            </div></FadeIn>
            <FadeIn delay={0.15}><div className="glow-card p-8 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-baseline gap-1 mb-1"><span className="text-4xl font-black text-white">₹249</span><span className="text-sm" style={{ color: '#475569' }}>/week</span></div>
                ...
                <ul className="space-y-3 mb-8">
                  {['50+ dream companies', 'Unlimited scanning', 'All sections unlimited', 'AI match + reasoning', 'Priority alerts', '4 premium mini-games'].map((f, i) => (
                    <li key={i} className="flex items-center gap-3 text-sm text-white">
                      <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px]" style={{ background: 'rgba(0,255,212,0.12)', color: '#00FFD4' }}>✓</span>{f}
                    </li>
                  ))}
                </ul>
      ...
      <FadeIn className="relative py-32 z-10 px-6 text-center">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-lg mb-10 max-w-xl mx-auto" style={{ color: '#475569' }}>Join freshers who let AI find their next role — automatically, hourly, relentlessly.</p>
          <Link href="/sign-up" className="btn-neon inline-flex items-center gap-3 px-12 py-5 text-lg">
            Launch Your RADAR <span className="text-2xl">🚀</span>
          </Link>
        </div>
      </FadeIn>
      ...
      <footer className="relative z-10 py-10 px-6 text-center" style={{ borderTop: '1px solid rgba(0,255,212,0.05)' }}>
        ...
        <p className="text-[11px]" style={{ color: '#334155' }}>© 2026 RADAR. AI Job Intelligence Engine. Built with obsession, powered by zero patience for bad job searches.</p>
      </footer>
    </main>
  );
}
